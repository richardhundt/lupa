...
_G[("package")].path=(";;./src/?.lua;./lib/?.lua;"..tostring(_G[("package")].path).."");
_G[("package")].cpath=(";;./lib/?.so;"..tostring(_G[("package")].cpath).."");

local newtable = loadstring(("return {...}"));
local bit = require(("bit"));

local core_meta = newtable();
core_meta.__index=_G;

local core = setmetatable(newtable(), core_meta);
setfenv((1), core);
...
__class=function(into, name, _from, _with, body) 
   ...
   _class.__apply=function(self,...) 
      if not(self:isa(_class)) then  
         error(("self parameter "..tostring(self).." is not a "..tostring(name)..""), (2));
       end 
      local obj = setmetatable(newtable(), self);
      if __op_in(("__init") , self) then  
   ...
   local uenv = getfenv((2));
   local fenv = setmetatable(Hash({ }), Hash({ __index = uenv }));
   uenv[name]=_class;
   into[name]=_class;
   setfenv(body, fenv);
   body(_class, _super);
...
__object=function(into, name, _from, _with, body) 
   ...
   local uenv = getfenv((2));
   local fenv = setmetatable(Hash({ }), Hash({ __index = uenv }));
   local wrap = function()  do return __class(into, (("#"))..(name), _from, _with, body) end  end;
   setfenv(wrap, fenv);
   local anon = wrap();
   local inst = anon();
   uenv[name]=inst;
...
Grammar=setmetatable(newtable(), Type);
Grammar.__name=("Grammar");
...
__import=function(_from, what) 
   ...
   if (what )and(( #(what) )>( (0))) then  
      ...
      local base = Hash({ });
      local curr = base;
      for i=(2), #(_from)  do local __break repeat 
         local name = _from[i];
         curr[name]=Hash({ });
         curr=curr[name];
       until true if __break then break end end 
      for k, v in __op_each(mod)  do local __break repeat 
         curr[k]=v;
       until true if __break then break end end 
      rawset(env, _from[(1)], base);
    end 
 end;
...
Type.isa=function(self, that) 
   if ((self )==( that ))or(( getmetatable(self) )==( that)) then    do return (true) end  end 
   if (type(self) )~=( ("table")) then    do return (false) end  end 
   if (__op_in(("__from") , self ))and( self.__from[that]) then    do return (true) end  end 
    do return (false) end
 end;
...
Object.isa=function(self, that) 
   local meta = getmetatable(self);
    do return (((self )==( that ))or(( meta )==( that )))or(
      ((__op_in(("__from") , meta ))and( ((meta.__from[that] )~=( (nil)))))) end
 end;
...
__class(self,"Lupa",{},{},function(self,super) 
   ...
   __grammar(self,"Grammar",function(self) 
      ...
      local function make_binop_bind(a1, a2, o, b) 
         a1=Lupa.Grammar:__get_expr():match(a1);
          do return a2:format(binops[o]:format(a1,b)) end
       end
      local function make_bind_expr(l, r) 
         if (#(l) )==( (1)) then  
             do return l[(1)]:format(r[(1)]) end
          end 
         local t = Array( );
         ...
         r=r:concat((","));
         l=l:concat((";"));
          do return ("local %s=%s;%s"):format(t,r,l) end
       end
      ...
      local function make_trait_decl(n,p,w,b) 
         local p, h = make_params(p);
          do return ("__trait(self,%q,{%s},function(self,%s) %s%s end);"):format(n,w,p,h,b) end
       end
      ...
      local function make_import_stmt(n,f) 
         ...
         for i=(1), #(f)  do local __break repeat  f[i]=quote(f[i]);  until true if __break then break end end 
          do return ("__import({%s},{%s});"):format(f:concat((",")), n:concat((","))) end
       end
      local function make_export_stmt(n) 
         for i=(1), #(n)  do local __break repeat  n[i]=quote(n[i]);  until true if __break then break end end 
          do return ("__export({%s})"):format(n:concat()) end
       end
      ...
      __rule(self,"try_stmt",
         ...
         ((s* __patt.P(("catch"))* idsafe* s*
            __patt.P(("("))* s* __patt.Cs( __patt.V("name") )* s* __patt.P((")"))* s*
      ...
      __rule(self,"import_stmt",
         __patt.Cs( ((__patt.P(("import"))* idsafe* s* (
             __patt.Ca( __patt.V("name")* (s* __patt.P((","))* s* __patt.V("name"))^0 )* s* __patt.P(("from"))* s* __patt.Ca( __patt.V("name")* (__patt.P(("::"))* __patt.V("name"))^0 )
            + __patt.Cc(Array())* __patt.Ca( __patt.V("name")* (__patt.P(("::"))* __patt.V("name"))^0 )
         )) )/(make_import_stmt) )
      );
      ...
      __rule(self,"for_in_stmt",
         __patt.Cs( __patt.P(("for"))* idsafe* s* __patt.V("name")* (s* __patt.P((","))* s* __patt.V("name"))^0* s* __patt.P(("in"))* idsafe* s*
      ...
      __rule(self,"meth_decl",
         __patt.Cs( ((__patt.P(("method"))* idsafe* s* __patt.V("name")* s*
         __patt.P(("("))* s* __patt.V("param_list")* s* __patt.P((")"))* s*
         __patt.P(("{"))* __patt.Cs( __patt.V("func_body")* s )* __patt.P(("}"))
         ) )/(make_meth_decl) )
      );
      __rule(self,"func_decl",
         __patt.Cs( ((__patt.P(("function"))* idsafe* s* __patt.Ca( __patt.V("name")* (s* __patt.P(("::"))* s* __patt.V("name"))^0 )* s*
         __patt.P(("("))* s* __patt.V("param_list")* s* __patt.P((")"))* s*
         __patt.P(("{"))* __patt.Cs( __patt.V("func_body")* s )* __patt.P(("}"))*
      ...
      __rule(self,"short_func",
         ((__patt.P(("->")) )/(("")))*
         ((s* __patt.P(("("))* s* __patt.V("param_list")* s* __patt.P((")")) + __patt.Ca( __patt.Cc(("_")) ))* s*
         __patt.P(("{"))* __patt.Cs( __patt.V("func_body")* s )* (__patt.P(("}"))
         )/(make_short_func))
      );
      ...
      __rule(self,"trait_decl",
         ...
         (__patt.P(("("))* s* __patt.V("param_list")* s* __patt.P((")")) + __patt.Ca( __patt.Cc(("...")) ))* s*
         (__patt.V("class_with") + __patt.Cc(("")))* s*
         __patt.P(("{"))* __patt.Cs( __patt.V("class_body")* s )* (__patt.P(("}"))
         )/(make_trait_decl)
      );
      ...
      __rule(self,"rest",
         __patt.Cs( __patt.C(__patt.P(("...")))* __patt.V("name")^-1 )
      );
      __rule(self,"stack",
         __patt.Cs(
      ...
      __rule(self,"param_list",
         __patt.Ca(
          __patt.Cs( __patt.V("name")* s )* (__patt.P((","))* __patt.Cs( s* __patt.V("name")* s ))^0* (__patt.P((","))* __patt.Cs( s* __patt.V("rest")* s ))^-1
         + __patt.V("rest")
         + __patt.Cc((nil))
         )
      );
      ...
      __rule(self,"member_expr",
         __patt.Cs(
          __patt.P(("."))* s* (__patt.V("name") )/((":__get_%1()"))
         + __patt.P(("::"))* s* __patt.P(("["))* s* __patt.V("expr")* s* (__patt.P(("]")) )/(("[%1]"))
         + __patt.P(("::"))* s* (__patt.V("name") )/((".%1"))
         + __patt.P(("["))* s* __patt.V("expr")* s* ((__patt.P(("]")) + __patt.P( syntax_error(("expected ']'")) )) )/((":__getitem(%1)"))
         )
      );
      ...
      __rule(self,"bind_expr",
         __patt.Cs( __patt.Ca( __patt.V("bind_list") )* s* __patt.P(("="))* s* ((
      ...
      __rule(self,"bind_binop_expr",
         __patt.Cs( __patt.C( __patt.V("bind_term") )* s* __patt.V("bind_binop")* s* (__patt.V("expr") )/(make_binop_bind) )
      );
      ...
      __rule(self,"bind_suffix",
         __patt.Cs(
          __patt.P(("."))*  s* __patt.V("name")* (s )/((":__set_%1(%%s)"))
         + __patt.P(("::"))* s* __patt.P(("["))* s* __patt.V("expr")* s* (__patt.P(("]")) )/(("[%1]=%%s"))
         + __patt.P(("::"))*  s* __patt.V("name")* (s )/((".%1=%%s"))
         + __patt.P(("["))* s* __patt.V("expr")*  s* (__patt.P(("]")) )/((":__setitem(%1,%%s)"))
         )
      );
      ...
      __rule(self,"rule_suffix",
         ...
         __patt.Cg( __patt.C(prod_oper)* __patt.V("rule_prod"),nil)^0) ,function(a,o,b,t) 
            if (o )==( ("=>")) then  
   ...
   __method(self,"compile",function(self,lupa, name) 
       do return Lupa.Grammar:match(lupa) end
    end);
 end);

eval=function(src) 
   local eval = assert(loadstring(Lupa:compile(src),(("=eval:"))..(src)));
   setfenv(eval, getfenv((2)));
    do return eval() end
 end;
...
local run = function(...) 
   ...
   if opt[("o")] then  
      ...
      lua=lua:gsub(("^%s*#![^\n]*"),(""));
      local code = ((("local self={};"))..(lua))..((" do return self end;"));
      local main = assert(loadstring(code,(("="))..(opt[("file")])));
      if opt[("b")] then  
         ...
         local main_env = setmetatable(Hash({ }), Hash({ __index = core }));
      ...
      for path in __op_each(Lupa.PATH:gmatch(("([^;]+)")))  do local __break repeat 
         if (path )~=( ("")) then  
            ...
            if file then  
               ...
               local lua = Lupa:compile(src);
               local code  = ((("local self={};"))..(lua))..((" do return self end"));
               local chunk = assert(loadstring(code, (("="))..(filepath)));
               local chunk_env = setmetatable(Hash({ }), Hash({ __index = core }));
